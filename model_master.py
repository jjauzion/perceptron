...
for l in range(topology_range["max_hidden_layer"]):
    for u in l_unit:
        for r in regul_range:
            ...
            test_score = []
            train_score = []
            while (delta_cost > 0.01 or total_iter < 500) and total_iter < 10000:
            ...
            stat[id_nb, 0] = id_nb
            stat[id_nb, 1] = test_score[-1]
            stat[id_nb, 2] = train_score[-1]
            stat[id_nb, 3] = np.prod([int(e) for e in topology.split(",")])
            stat[id_nb, 4] = r
            stat[id_nb, 5] = model.nb_iteration_ran
